...
function CustomModal(props) {
  ...
  const [currentMinVote, setCurrentMinVote] = useState();
  ...
  const updateMinVoteValue = () => {
    setCheckValid(true);
    if (
      (inputType === 'min_vote' &&
        !phoneNumberRegex.test(currentMinVote.toString().trim())) ||
      !currentMinVote.toString().trim()
    ) {
      return false;
    }
    setCheckValid(false);
    onUpdate(currentMinVote);
    setCurrentValue('');
  };
      ...
      >
        ...
        {inputType === 'min_vote' && (
          <div className="inputCustomValue">
            ...
            <Input
              value={currentMinVote ? currentMinVote : undefined}
              onChange={event =>
                setCurrentMinVote(parseInt(event.target.value))
              }
            />
            <div className="inputCustomValueText">候補</div>
            {!phoneNumberRegex.test(currentMinVote) && checkValid && (
              <span className={styles.errorNotice}>
